...
public sealed partial class KubernetesClientSession
{
    private async ValueTask<KubernetesMetricsSnapshot> ReadPrometheusUsageAsync(KubernetesMetricsRequest request, CancellationToken cancellationToken)
    {
        ...
        string path = $"api/v1/namespaces/{provider.Namespace}/services/{Uri.EscapeDataString(service)}/proxy/api/v1/query";
        ...
        try
        {
            ...
            await ReadInstantUsageAsync(path, cpu, request.Kind, request.Namespace, true, entries, lifetime.Token).ConfigureAwait(false);
            await ReadInstantUsageAsync(path, memory, request.Kind, request.Namespace, false, entries, lifetime.Token).ConfigureAwait(false);
    ...
    private async Task ReadInstantUsageAsync(string path, string query, KubernetesMetricsKind kind, string? expectedNamespace, bool cpu,
        Dictionary<(string Namespace, string Name, string Container), KubernetesUsageEntry> entries, CancellationToken cancellationToken)
    {
        ...
        foreach (JsonElement item in result.EnumerateArray())
        {
            ...
            entries[key] = entry with
            {
                ...
                CpuCores = cpu ? usage : entry.CpuCores,
                MemoryBytes = cpu ? entry.MemoryBytes : usage
            };
        }
    }
}
